...
#define USE_XIPCPY 0
#if PICO_ON_DEVICE && !THUMBY_NATIVE
...
#if THUMBY_NATIVE
extern "C" void V_ClearOverlay(void);
#endif
...
//    uint8_t col_height[pdi.w];
    ...
    do {
        ...
        if (!fracstep) {
            ...
            fracstep = (c.scale == 0) ? DDA_UP_SHIFT(pspriteiscale) : 0x10000;
...
#if THUMBY_NATIVE
static void draw_splash_shrunk(int patch_num, int src_top, int src_h, uint8_t *dest, int dst_top, int dst_h) {
    patch_decode_info pdi;
    get_patch_decoder(patch_num, &pdi);
    int src_w = patch_width(pdi.patch);
    const uint16_t *col_offsets = pdi.col_offsets;
    const uint8_t *patch_decoder_table = get_patch_decoder_table(patch_num, pdi.decoder);

    // Precompute row mapping (nearest-neighbor): dst row -> src row
    uint8_t row_src[SCREENHEIGHT];
    for (int dy = 0; dy < dst_h; dy++) {
        row_src[dy] = (uint8_t)((dy * src_h) / dst_h);
    }

    for (int dx = 0; dx < SCREENWIDTH; dx++) {
        int sx = (dx * src_w) / SCREENWIDTH;
        if (sx >= src_w) sx = src_w - 1;

        uint16_t col_offset = col_offsets[sx];
        if (0xff == (col_offset >> 8)) {
            col_offset = col_offsets[col_offset & 0xff];
        }

        // Decode entire source column into stack buffer
        uint8_t col_buf[256];
        th_bit_input bi;
        if (patch_byte_addressed(pdi.patch)) {
        ...
        for (int sy = 0; sy < src_h; sy++) {
            ...
            col_buf[sy] = prev_pixel;
        }

        // Sample into destination via row_src mapping
        uint8_t *dp = dest + dx;
        for (int dy = dst_top; dy < dst_h; dy++) {
            *dp = col_buf[row_src[dy]];
            dp += SCREENWIDTH;
        }
    }
}
...
static void draw_splash(int patch_num, int top, int bottom, uint8_t *dest, int single_col = -1) {
    ...
    int h = patch_height(pdi.patch);
#if THUMBY_NATIVE
    // Oversized patches (320x200 title screens etc.) must be scaled down
    if (single_col < 0 && top == 0 && (w > SCREENWIDTH || h > SCREENHEIGHT)) {
        /* Draw splash into full screen (128x128), not just the 3D view
         * area. This covers the HUD region so no blank strip shows. */
        draw_splash_shrunk(patch_num, 0, h, dest, 0, SCREENHEIGHT);
        return;
    }
#endif
    assert(w == 320 && h == 200);
...
#if THUMBY_NATIVE
        /* Draw splash to the FULL 128x128 buffer so the title/credits
         * fill the screen with no HUD artefacts visible underneath. */
        draw_splash(patch_num, 0, SCREENHEIGHT, frame_buffer[render_frame_index]);
#else
...
void draw_stbar_on_framebuffer(int frame, boolean refresh) {
    refresh = true; // always refresh on Thumby
    V_BeginPatchList(vpatchlists->framebuffer);
    ST_drawWidgets(refresh);
    // v_overlay_buf receives the draws when v_patch_stride==320
    V_RestoreBuffer();
...
void draw_fullscreen_background(int top, int bottom) {
    ...
    int patch_num = 0;
#if THUMBY_NATIVE
    byte *top_pixel = render_frame_buffer + top * SCREENWIDTH;
#else
...
#if THUMBY_NATIVE
/* THUMBY_NATIVE: single-core display, core1 is audio only.
 * The original save-pause used semaphore handshaking with core1's
 * display loop which doesn't exist here. Just set VIDEO_TYPE_SAVING
 * so present_frame shows the save indicator, and return. */
