...
#if THUMBY_NATIVE
        {
            extern void thumby_melt_start(void);
            extern int  thumby_melt_is_active(void);

            boolean did_wipe = D_Display();

            if (did_wipe) {
                /* g_fb still holds the old screen — snapshot it now,
                 * before I_FinishUpdate overwrites with the new frame. */
                thumby_melt_start();
                /* Kill pd_render's scanline wipe state machine —
                 * we handle the visual effect ourselves at RGB565. */
                wipestate = 0;
            }

            I_FinishUpdate();   /* presents with melt composite if active */

            /* Keep the game ticking while the melt plays out. */
            while (thumby_melt_is_active()) {
                I_StartFrame();
                TryRunTics();
...
#if THUMBY_NATIVE
    { extern int boot_active; boot_active = 0; }
#endif
