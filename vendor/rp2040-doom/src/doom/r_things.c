...
#if THUMBY_NATIVE
/* BASEYCENTER positions the weapon vertically. It's ONLY used for
 * psprites (line 882). In vanilla (320x200, pspritescale=1.0),
 * BASEYCENTER=100 puts the weapon filling the lower screen half.
 *
 * At our resolution (128x128, pspritescale=128/320=0.4):
 *   sprtopscreen = centery - texturemid * pspritescale
 *   texturemid   = BASEYCENTER + 0.5 - (WEAPONTOP - topoffset)
 *
 * With WEAPONTOP=32, typical topoffset=30:
 *   texturemid = BASEYCENTER - 1.5
 *
 * Tunable — higher = weapon higher on screen (larger texturemid
 * pushes sprtopscreen up via centery - texturemid*scale). */
#define BASEYCENTER            57
#else
...
void R_DrawPSprite(pspdef_t *psp) {
    ...
    // calculate edges of the shape
#if THUMBY_NATIVE
    /* psp->sx is in vanilla units (FRACUNIT ≈ 1/320th screen).
     * Center using vanilla half-width (160). */
    ...
    // off the right side
    if (x1 > viewwidth) {
#if THUMBY_NATIVE && !PICO_ON_DEVICE
        fprintf(stderr, "psprite clipped RIGHT: x1=%d vw=%d tx=%d sx=%d off=%d scale=%d\n",
            x1, viewwidth, tx>>FRACBITS, psp->sx>>FRACBITS, sprite_offset(lump)>>FRACBITS, pspritescale);
#endif
        return;
    }
    ...
    if (x2 < 0) {
#if THUMBY_NATIVE && !PICO_ON_DEVICE
        fprintf(stderr, "psprite clipped LEFT: x2=%d tx=%d width=%d sx=%d\n",
            x2, tx>>FRACBITS, sprite_width(lump)>>FRACBITS, psp->sx>>FRACBITS);
#endif
        return;
    }
