...
/* Original kilograham uint16_t encoding for vanilla rp2040-doom
 * (requires custom linker script to constrain placement). */
#include <assert.h>
#if !PICO_RP2350
...
#else
/* THUMBY_NATIVE and host: full pointers. Removes the 256 KB window
 * constraint and the linker-layout fragility entirely. Costs a few
 * KB of extra BSS for the wider thinker/mobj fields — well within
 * our headroom on RP2350 (520 KB SRAM vs RP2040's 264 KB). */
#define SHORTPTR_BASE 0  /* defined for static_asserts that reference it */
