...
#if THUMBY_NATIVE
/* THUMBY_NATIVE: All 2D overlays (HUD, menu, intermission text) use
 * 320×200 coordinates. We render them into a 320×200 staging buffer
 * and downsample onto the native framebuffer. v_patch_stride controls
 * the dest row stride — set to 320 for the staging buffer, 128 for
 * native. vpatch_clip_* use the stride's coordinate space. */
int v_patch_stride = 320;
uint8_t vpatch_clip_top = 0, vpatch_clip_bottom = 200;

/* The staging buffer. Index 0 = transparent (background shows through). */
uint8_t v_overlay_buf[320 * 200];

void V_ClearOverlay(void) {
    /* Full clear — the STBAR is redrawn every frame (refresh=true)
     * so no need to preserve any region. */
    memset(v_overlay_buf, 0, sizeof(v_overlay_buf));
}

/* Composite overlay onto native 128xH frame_buffer. Downsamples
 * 320×200 → 128×128 via nearest-neighbor. Transparent pixels (0)
 * are skipped. */
void V_CompositeOverlay(uint8_t *dest)
{
    for (int dy = 0; dy < SCREENHEIGHT; dy++) {
        int sy = (dy * 200) / SCREENHEIGHT;
        const uint8_t *srow = v_overlay_buf + sy * 320;
        uint8_t *drow = dest + dy * SCREENWIDTH;
...
#else
#define v_patch_stride SCREENWIDTH
uint8_t vpatch_clip_top = 0, vpatch_clip_bottom = SCREENHEIGHT;
#endif
...
void V_DrawPatchList(const vpatchlist_t *patchlist) {
    ...
    for (int l = 1; l < patchlist[0].header.size; l++) {
        int px = patchlist[l].entry.x;
        int py = patchlist[l].entry.y;
        if (px < 0 || py < 0 || px >= v_patch_stride) continue;
        uint8_t *orig = dest_screen + py * v_patch_stride + px;
        ...
        if (px + w > v_patch_stride) w = v_patch_stride - px;
        if (w <= 0) continue;
...
#pragma GCC diagnostic pop
        ...
        switch (type) {
            case vp4_runs_clipped:
                for(;skip_top--; desttop += v_patch_stride) {
                    uint8_t gap;
                    int p = 0;
                    while (0xff != (gap = *data++)) {
                        p += gap;
                ...
                // fall thru
            case vp4_runs:
                for (; h > 0; h--, desttop += v_patch_stride) {
                    uint8_t *p = desttop;
            ...
            case vp4_alpha_clipped:
                data += ((w + 1) / 2) * skip_top;
                desttop += v_patch_stride * skip_top;
                // fallthru
            case vp4_alpha:
                for (; h > 0; h--, desttop += v_patch_stride) {
                    uint8_t *p = desttop;
...
void V_DrawPatchN(int x, int y, vpatch_handle_large_t patch_handle, int repeat) {
...
#ifdef RANGECHECK
#if THUMBY_NATIVE
    if (x < 0 || x + vpatch_width(patch) > v_patch_stride
        || y < 0 || y + vpatch_height(patch) > (v_patch_stride == 320 ? 200 : SCREENHEIGHT)) {
        /* skip silently — overlay patches may exceed native screen */
    }
#else
    if (x < 0
    ...
    if (vpatchlist[0].header.size <= vpatchlist[0].header.max) {
        /* Coordinates stay in 320x200 space — the overlay staging
         * buffer is 320 wide, so V_DrawPatchList writes at the
         * correct position. Downsampling to 128x128 happens later
         * in V_CompositeOverlay. */
...
void V_DrawPatchFlipped(int x, int y, vpatch_handle_large_t patch_handle) {
    ...
    col = 0;
    desttop = dest_screen + y * v_patch_stride + x;
    ...
    for (; col < w; x++, col++, desttop++) {
        ...
        while (column->topdelta != 0xff) {
            source = (byte *) column + 3;
            dest = desttop + column->topdelta * v_patch_stride;
            count = column->length;
...
void V_DrawShadowedPatch(int x, int y, patch_t *patch)
{
    ...
    col = 0;
    desttop = dest_screen + y * v_patch_stride + x;
    desttop2 = dest_screen + (y + 2) * v_patch_stride + x + 2;
    ...
    for (; col < w; x++, col++, desttop++, desttop2++)
    {
        ...
        while (column->topdelta != 0xff)
        {
            source = (byte *) column + 3;
            dest = desttop + column->topdelta * v_patch_stride;
            dest2 = desttop2 + column->topdelta * v_patch_stride;
            count = column->length;
...
void V_DrawBlock(int x, int y, int width, int height, pixel_t *src) {
    ...
    dest = dest_screen + y * v_patch_stride + x;
...
#if THUMBY_NATIVE
    /* When v_patch_stride is 320 (overlay mode), direct all
     * patch drawing into the overlay staging buffer. */
    if (v_patch_stride == 320) {
        dest_screen = v_overlay_buf;
    } else {
