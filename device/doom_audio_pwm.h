 ...
 * frame from nes_audio_render(). Buffer size is sized so a 30 fps
 * frame (735 samples) fits comfortably with margin.
 */
#ifndef THUMBYDOOM_AUDIO_PWM_H
#define THUMBYDOOM_AUDIO_PWM_H
...
void doom_audio_pwm_push(const int16_t *samples, int n_samples);

/* How much room is currently in the ring buffer (in samples). */
int  doom_audio_pwm_room(void);
